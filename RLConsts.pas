...
const
  CS_CopyrightStr = 'Copyright © 1999-2009 Fortes Informática';
  CS_ProductTitleStr = 'FortesReport (Open Source)';
  CS_URLStr = 'http://www.fortesreport.com.br';
  CS_AuthorNameStr = 'Ronaldo Moreira, Eduardo Richeli, Márcio Martins, Tiago Moraes, Fred de Castro Silva';
...
procedure SetVersion(Commercial, Release: Integer; const Fortes: string);
...
procedure SetVersion(Commercial, Release: Integer; const Fortes: string);
begin
  KnownCommercialVersion := Commercial;
  KnownReleaseVersion := Release;
  KnownCommentVersion := Fortes;
  CheckVersion(nil);
end;
...
begin
  ...
  if (KnownCommercialVersion <> KnownCommercialVersion) or
    (KnownReleaseVersion <> ReleaseVersion) or
    (KnownCommentVersion <> CommentVersion) then
    if not ReportServiceMode then
    ShowMessage(
    'Sistema não está de acordo com a versão do FortesReport.'#13 + 
    #13 + 
    'Versão atual ' + IntToStr(CommercialVersion) + '.' + IntToStr(ReleaseVersion) + CommentVersion);
end;
