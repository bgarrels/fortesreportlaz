...
begin
  ...
  case AObj.Layout of
    MetaTextLayoutTop:
    begin
      T := FPrintSize.Y - (AObj.BoundsRect.Top + W.Y);
      T := T - MagicY;
    end;
    MetaTextLayoutCenter:
    begin
      T := (AObj.BoundsRect.Top + AObj.BoundsRect.Bottom - W.Y) div 2;
      T := FPrintSize.Y - (T + W.Y);
    end;
  else
    T := FPrintSize.Y - AObj.BoundsRect.Bottom;
  end;
...
begin
  ...
  Writeln('/MediaBox [ 0 0 ' + PDF_FloatToStr(PDF_PixelsToPoints(FPrintSize.X)) +
    ' ' + PDF_FloatToStr(PDF_PixelsToPoints(FPrintSize.Y)) + ' ]');
  Write('/Kids [ ');
...
initialization
  ...
  begin
    ...
    Author := '';
    KeyWords := '';
    Creator := CS_ProductTitleStr + ' v' + IntToStr(CommercialVersion) +
      '.' + IntToStr(ReleaseVersion) + CommentVersion + ' \251 ' + CS_CopyrightStr;
    Producer := '';
  end;
