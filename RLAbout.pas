...
{$endif}
  ...
  LabelVersion := TLabel.Create(Self);
  ...
  begin
    ...
    Height := 13;
{$ifdef VCL}
    Caption := IntToStr(CommercialVersion) + '.' + IntToStr(ReleaseVersion) + CommentVersion + ' VCL';
{$endif}
{$ifdef CLX}
    Caption := IntToStr(CommercialVersion) + '.' + IntToStr(ReleaseVersion) + CommentVersion + ' CLX';
{$endif}
    Font.Name := 'helvetica';
    Font.Color := clBlack;
    Font.Height := 13;
  ...
  LabelCopyright := TLabel.Create(Self);
