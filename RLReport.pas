...
{$ifend}
  ...
  public
    ...
    function GetRelativeColumnFooterRowNoSummary: Integer;
...
begin
  ...
  if BandType = btSummary then
    ...
    footr := Pager.GetRelativeColumnFooterRowNoSummary;
...
begin
  ...
  AAvailable := Pager.GetRelativeColumnFooterRowNoSummary - pagerrow;
  Result := (AHeight <= AAvailable);
end;
  ...
  try
    ...
    case B.Completion of
      ...
      begin
        ...
        iLast := GetRelativeColumnFooterRowNoSummary;
...
var
  R: Integer;
begin
  R := GetRelativeColumnFooterRowNoSummary;
  Result := FRelativePagerRow < R;
  if Result then
    FRelativePagerRow := R;
end;
...
begin
  ...
  for I := 0 to L.Count - 1 do
    if TObject(L.Items[I]) is TRLCustomBand then
      with TRLCustomBand(L.Items[I]) do
        if {AlignToBottom and} Visible then ///quarentena: {} por causa da rafaela
          Inc(Result, Height);
end;
...
begin
  ...
  Result := GetRelativeFooterRow - GetSummaryHeight;
end;
...
function TRLCustomPager.GetRelativeColumnFooterRowNoSummary: Integer;
begin
  // neste caso, nao levo em consideracao os sumarios, pois o cara que me pergunta
  // nao pretende imprimi-los.
  Result := GetRelativeFooterRow - GetColumnFooterHeight;
end;
